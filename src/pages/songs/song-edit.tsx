...
const SongEdit = () => {
  ...
  const pageTitle = isNewSong ? "Add New Song" : "Edit Song"
  ...
  const [audioPreview, setAudioPreview] = useState<null | string>(null)
  const [imagePreview, setImagePreview] = useState<null | string>(null)

  // Fetch song data if editing
  const { data: song, isLoading: songLoading } = useGetQuery({
    key: ["song", id],
    url: `/api/songs/${id}`,
    options: {
      enabled: !isNewSong,
      initialData: isNewSong
        ? null
        : {
  ...
  // Set form data when song data is loaded
  useEffect(() => {
    if (song && !isNewSong) {
      setFormData({
        title: song.title || "",
      ...
      if (song.audioUrl) {
        setAudioPreview(song.audioUrl)
      }

      if (song.coverImageUrl) {
        setImagePreview(song.coverImageUrl)
      }
    }
  }, [song, isNewSong])
  ...
  const handleRemoveFile = (fileType: "audio" | "image") => {
    if (fileType === "audio") {
      setFormData((prev) => ({
        ...prev,
        audioFile: null,
  ...
  const validate = () => {
    ...
    if (isNewSong && !formData.audioFile) {
  ...
  const isLoading = songLoading || createMutation.isPending || updateMutation.isPending

  if (songLoading && !isNewSong) {
    return <div className="p-6 text-center">Loading song data...</div>
  }
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-600">{isNewSong ? "Add a new song to the system" : "Update song information"}</p>
        </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
        <form onSubmit={handleSubmit} className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-6">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Audio File {isNewSong && "*"}</label>
                <div
              ...
              {!isNewSong && (
                <div className="bg-[#EEF4F7] p-4 rounded-lg">
                  <h3 className="text-sm font-medium text-gray-700 mb-2">Song Statistics</h3>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <p className="text-xs text-gray-500">Total Plays</p>
                      <p className="text-lg font-medium">{song?.plays?.toLocaleString() || 0}</p>
                    </div>
                    <div>
          ...
          <div className="mt-8 flex justify-end">
            ...
            >
              {isLoading ? "Saving..." : isNewSong ? "Add Song" : "Update Song"}
            </button>
          </div>
        </form>
      </div>
